...
const LogBuffer& LogStream::get_buffer() {
    return buffer_;
}
...
Logging::Logging(const string log_name, int flush_interval) : flush_interval_(flush_interval), is_running_(false),
    log_name_(log_name), thread_(bind(&thread_func, this), "Logging"), mutex_lock_(), cond_(),
...
void Logging::append(const char *log_line, const int len) {
    ...
    if (cur_buffer_->get_available_size() > len) cur_buffer_->append(log_line, len);
        ...
        cur_buffer_.reset();
        if (next_buffer_) cur_buffer_ = move(next_buffer_);
...
void Logging::thread_func() {
    ...
    while (is_running_) {
        assert(log_buffer1 && log_buffer1->length == 0);
        assert(log_buffer2 && log_buffer2->length == 0);
...

        {
            ...
            cur_buffer_ = move(log_buffer1);
            buffer_list_output.swap(buffer_list_);
            if (!next_buffer_) next_buffer_ = move(log_buffer2);
        }
        ...
        if (!log_buffer1) {
            assert(!buffer_list_output.empty());
            log_buffer1 = buffer_list_output.pop_back();
            log_buffer1.reset();
        }

        if (!log_buffer2) {
            assert(!buffer_list_output.empty());
            log_buffer2 = buffer_list_output.pop_back();
            log_buffer2.reset();
        }
